// ── Currency Rate (KHR per USD) ─────────────────────────────────
window.KHR_RATE = (function(){
  try { const r = Number(localStorage.getItem('cambo_khr_rate')); return r > 0 ? r : 4100; } catch { return 4100; }
})();
function getKhrRate(){ return window.KHR_RATE; }
...
// ── Normalize default products for product-manager ──
(function() {
  var defaults = products.map(function(p) {
    return {
      id:       p.id,
      name:     p.name,
      subName:  p.detail || '',
  ...
  if (window.pmInitProducts) {
    window.pmInitProducts(defaults);
  } else {
    window.__camboProducts = defaults;
  }
})();

// Expose renderProductGrid so product-manager can call it
...
function getFilteredProducts() {
  // Always use managed products (IndexedDB via product-manager) when available
  var src;
  if (window.__camboProducts && window.__camboProducts.length) {
    src = window.__camboProducts
  ...
  return activeCategory === "All" ? src : src.filter(function(p){ return p.category === activeCategory; });
}
...
function addItem(productId, qty, price, unit) {
  ...
  const managedList = (window.__camboProducts && window.__camboProducts.length)
    ...
    : [];
  const product = managedList.find((p) => String(p.id) === String(productId))
...
function updateRielDisplay() {
  const el = document.getElementById('rielTotalDisplay');
  if (!el) return;
  const total = getGrandTotal();
  const rate  = getKhrRate();
  el.textContent = Math.round(total * rate).toLocaleString() + '៛';
}
...
function renderTable() {
  ...
  els.itemsTableBody.innerHTML = items.map((item) => `
    <tr class="item-row${currentEditRowId === item.rowId ? ' item-row-active' : ''}" data-rowid="${item.rowId}">
      <td>
        <div class="row-name-cell">
          <span class="row-prod-name">${escapeHtml(item.name)}</span>
        </div>
      </td>
      <td class="row-center">
        <div class="row-qty-inline">
          <button class="row-qty-btn" data-rowid="${item.rowId}" data-dir="-1" type="button">−</button>
          <span class="row-qty-val">${item.qty}</span>
          <button class="row-qty-btn" data-rowid="${item.rowId}" data-dir="1" type="button">+</button>
        </div>
      </td>
      <td class="row-center">${formatDisplayMoney(item.price)}</td>
      <td class="row-center">${formatDisplayMoney(item.discount)}</td>
      <td class="row-center row-subtotal">${formatDisplayMoney(item.subtotal)}</td>
      <td class="row-actions-cell">
        <button class="row-act-btn row-edit-btn" data-rowid="${item.rowId}" type="button" title="Edit">✏️</button>
        <button class="row-act-btn row-del-btn"  data-rowid="${item.rowId}" type="button" title="Delete">🗑️</button>
      </td>
    </tr>
  `).join("");
...
function attachProductRowActions() {
  ...
  // Inline QTY +/- buttons
  document.querySelectorAll(".row-qty-btn").forEach((btn) => {
    btn.addEventListener("click", () => {
      const rowId = Number(btn.dataset.rowid);
      ...
      item.subtotal = Math.max(0, item.qty * item.price - item.discount);
      renderTable();
      if (currentEditRowId === rowId) openProductDrawer(rowId);
    });
  });

  // Click product name → open drawer too
  document.querySelectorAll(".row-prod-name").forEach((el) => {
    el.style.cursor = "pointer";
    el.addEventListener("click", () => {
      const rowId = Number(el.closest("tr").dataset.rowid);
      openProductDrawer(rowId);
    });
  });
}
...
function handlePrint() {
  ...
  if (typeof ReceiptPrinter === "undefined") {
    toast("Receipt printer script is missing.", "error");
    return;
  }
  ...
  const addressText = [detailAddress, province].filter(Boolean).join(" : ") || "-";
  const receiptNo = getReceiptNoValue();
  const pageValue = (els.page?.value || "-").trim() || "-";
  const closeByValue = (els.closeBy?.value || "-").trim() || "-";

  const receiptData = {
    title: "វិក័យប័ត្រ",
    paperWidth: "80mm",
    showQR: !!qrMeta,
    date: formatDateForReceipt(els.date?.value || ""),
    customer: (els.customer?.value || "-").trim() || "-",
    phone: (els.phone?.value || "-").trim() || "-",
    address: addressText,
    deliveryName: (els.deliveryName?.value || "-").trim() || "-",
    ...
    qrImage: qrMeta ? getAssetUrl(qrMeta.src) : "",
    ...
    grandRiel: Math.round(getGrandTotal() * getKhrRate())
  };
...
async function handleCopy() {
  ...
  if (typeof CopyReceipt === "undefined") {
    toast("Copy receipt script is missing.", "error");
    return;
  }
  ...
  try {
    await CopyReceipt.copy(copyData);
...
function buildOrderPayload() {
  ...
  return {
    ...
    total: getGrandTotal(),
    totalRiel: getGrandTotal() * getKhrRate()
  };
}
...
async function handleShareInvoice() {
  ...
  if (typeof ShareReceipt === "undefined") {
    toast("Share receipt script is missing.", "error");
    return;
  }
  ...
  const shareData = {
    title: "វិក័យប័ត្រ",
    date: formatDateKH(els.date?.value),
    customer: (els.customer?.value || "-").trim() || "-",
    phone: (els.phone?.value || "-").trim() || "-",
    address: addressText,
    deliveryName: (els.deliveryName?.value || "-").trim() || "-",
  ...
  try {
    ...
    const result = await ShareReceipt.share(shareData, {
      target: els.printArea
    });
...
function clearOrderForm() {
  ...
  // Reset searchable combo visible inputs
  document.querySelectorAll('.sc-wrap .sc-inp').forEach(function(inp){ inp.value = ''; });
  // Reset priority
...
function buildShareReceiptHTML() {
  ...
  const grandRiel = Math.round(grand * getKhrRate());
    ...
    <div class="share-poster ${hasBottomBlock ? 'has-bottom-block' : 'trim-bottom'}">
      <div class="share-content">
        <div class="share-head">
          <div class="share-order-num">${receiptNo ? escapeHtml(receiptNo) : ''}</div>
...
function buildReceiptHTML() {
  ...
  const itemsTotal = getItemsTotal();
  const grand = itemsTotal + deliveryFee;
  const grandRiel = grand * getKhrRate();
  ...
  return `
    <div class="print-card receipt-paper">
      ...
      <div class="receipt-pay-row receipt-grand-row">
        <div class="receipt-pay-method">
          <span>ការទូទាត់:</span>
          <strong class="receipt-pay-badge">${paymentText !== '-' ? escapeHtml(paymentText) : '—'}</strong>
        </div>
